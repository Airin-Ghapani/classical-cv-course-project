...
Multi-scale luminance + opponent-color gradients with local orientation coherence

Sobel + ridge-fusion comparison

Super-resolution

Edge-adaptive bicubic/Lanczos fusion with mild anti-ringing smoothing

Bicubic + learned patch-residual comparison

Counting

Multi-scale LoG seeds with image-adaptive single-cell area correction

Otsu connected components + ridge count calibrator

Color constancy

Robust fourth-order Gray-Edge with dark/highlight rejection

Gray-world, standard Gray-Edge + ridge illuminant regressor

Compression capstone

Activity-adaptive 8×8 DCT quantization with rate accounting

Uniform DCT, Pillow JPEG, and KLT/PCA transform
...
Final classical method

Small learned comparison

Boundary detection — mean ODS F1
...
Super-resolution ×2 — mean PSNR / SSIM

Bicubic: 23.648 dB / 0.7163

23.652 dB / 0.7177

~24.45 dB / 0.758

Counting — mean absolute count error
...
Color constancy — mean / median angular error

Gray-world: 8.44° / 8.61°
...
The notebook intentionally does not embed the BSDS source images in its saved outputs. Figures are regenerated locally after the dataset setup step.
...
Dataset imagery is not embedded in this repository version; plots are regenerated on execution.
